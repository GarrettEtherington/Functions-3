...
const numsDivByTwo = numbers.map(num => num / 2)
console.log(numsDivByTwo)
...
const citiesI = cities.filter(name => name.endsWith `i`)
console.log(citiesI)
...
fruits.forEach((color, index) => { 
  if(color.color === "red"){
    console.log(`the fruit with index ${fruits.findIndex(fruit => fruit.name === "apple")} is an apple.`)
  } else if(color.color === "orange"){
    console.log(`the fruit with index ${fruits.findIndex(fruit => fruit.name === "orange")} is an orange`)
  } else{
    console.log(`The fruit with index ${index} is neither an apple or orange.`)
  }
})
...
const riceIsGood = foods.filter(element => element.tags.includes(`rice`)).reduce((runTot, price) => runTot + price.price, 0)

console.log(riceIsGood)
