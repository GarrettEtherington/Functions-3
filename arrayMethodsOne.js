...
// CODE HERE
const totalPopulation = populations.reduce((accumulator, currentValue) => accumulator + currentValue)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(arrayCP => arrayCP.CP > 200)
...
const arrayM = orders.map(price => price.price * (price.tax + 1))
console.log(arrayM)
...
const bobsNums = purchases.filter(bobO => bobO.owner === `Bob`).reduce((bobTotal, bobValue) => bobTotal + bobValue.price,0)
console.log(bobsNums)